...
public class MysqlHandler: AppHandlers {
	...
	private func restartMysql() {
		...
		if(self.asyncTaskPid == nil) {
			...
			self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "MYSQL Async task already started")
			if(kill(self.asyncTaskPid!, 0) != 0) {
				
				self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "MYSQL Async task already started but does not work!")
	...
	public required init(logger: Logger, configFilePath: String, moduleConfig: Section?) {
		...
		super.init(logger: logger, configFilePath: configFilePath, moduleConfig: moduleConfig)
		self.logger.writeLog(Logger.LogLevels.ERROR, message: "MYSQL extension only works swift >= 3 build.")
	}
#endif
}
