//
//  DovecotHandler.swift
//  IORunner/Extensions/DovecotHandler
//
...
public class DovecotHandler: AppHandlers {
	...
	public override func forStart() {
		...
		self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "DOVECOT extension registered!")
	...
	private func checkDovecotProcess() -> Bool {
		...
		if let currentProcessName = moduleConfig?["ProcessName"] {
			...
			if(self.processStatus.count > 0) {
				...
				self.logger.writeLog(level: Logger.LogLevels.ERROR, message: "Warning Process DOVECOT does not working!")
				return false
			}
		}
	...
	private func restartDovecot() {
		
		self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "Restarting DOVECOT")
		...
		if(self.asyncTaskPid == nil) {
			...
			self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "DOVECOT Async task already started")
			if(kill(self.asyncTaskPid!, 0) != 0) {
				
				self.logger.writeLog(level: Logger.LogLevels.WARNINGS, message: "DOVECOT Async task already started but does not work!")
